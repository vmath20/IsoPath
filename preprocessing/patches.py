...
tcga_brca_slides = slides_df[slides_df['Project ID'] == 'TCGA-BRCA']
brca_embeddings = []
sampled_brca_slides = tcga_brca_slides.sample(n=num_slides, random_state=42)
sampled_brca_slides.to_csv('sampled_brca_slides.csv', index=False)
brca_indices_df = pd.DataFrame()
preprocessed_patches_dir = "preprocessed_patches_BRCA"

tcga_lusc_slides = slides_df[slides_df['Project ID'] == 'TCGA-LUSC']
lusc_embeddings = []
sampled_lusc_slides = tcga_lusc_slides.sample(n=num_slides, random_state=42)
sampled_lusc_slides.to_csv('sampled_lusc_slides.csv', index=False)
lusc_indices_df = pd.DataFrame()
preprocessed_patches_dir = "/lotterlab/users/vmishra/preprocessed_patches_LUSC"

tcga_luad_slides = slides_df[slides_df['Project ID'] == 'TCGA-LUAD']
luad_embeddings = []
sampled_luad_slides = tcga_luad_slides.sample(n=num_slides, random_state=42)
sampled_luad_slides.to_csv('sampled_luad_slides.csv', index=False)
luad_indices_df = pd.DataFrame()
preprocessed_patches_dir = "/lotterlab/users/vmishra/preprocessed_patches_LUAD"
...
preprocess_and_save_patches(sampled_brca_slides, patch_size=224, num_patches=num_patches_per_slide, category_label="BRCA", save_dir=preprocessed_patches_dir)
preprocess_and_save_patches(sampled_lusc_slides, patch_size=224, num_patches=num_patches_per_slide, category_label="LUSC", save_dir=preprocessed_patches_dir)
preprocess_and_save_patches(sampled_luad_slides, patch_size=224, num_patches=num_patches_per_slide, category_label="LUAD", save_dir=preprocessed_patches_dir)
